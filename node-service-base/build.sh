#!/usr/bin/env bash
# Build bozemanpass/node-service-base
...
STACK_CONTAINER_BUILD_TAG=${STACK_CONTAINER_BUILD_TAG:-bozemanpass/node-service-base:stack}
...
if [ "$STACK_CONTAINER_BUILD_TAG" != "bozemanpass/node-service-base:stack" ]; then
  cat <<EOF
